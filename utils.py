...
from pyrogram.errors import FloodWait, UserNotParticipant, UserIsBlocked, PeerIdInvalid, InputUserDeactivated
...
# --- BLOCKING FUNCTION MOVED TO THREAD ---
...
async def get_poster(query, bulk=False, id=False, file=None):
    # Runs the blocking IMDB code in a separate thread so the bot doesn't freeze
    return await asyncio.to_thread(_get_poster_sync, query, bulk, id, file)
...
async def broadcast_messages(user_id, message):
    try:
        ...
        await asyncio.sleep(e.value) # Fixed: e.x to e.value for standard Pyrogram, or keep e.x if pyrofork specific
...
async def search_gagala(text):
    # Fixed: Replaced blocking requests with aiohttp
    usr_agent = {
...
async def check_verification(bot, userid):
    ...
    if not await db.is_user_exist(user.id):
        await db.add_user(user.id, user.first_name)
        await bot.send_message(LOG_CHANNEL, script.LOG_TEXT_P.format(user.id, user.menti
